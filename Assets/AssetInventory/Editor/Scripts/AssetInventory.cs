...
namespace AssetInventory
{
    ...
    {
        public const string TOOL_VERSION = "1.9.0";
        ...
        {
            string cachePath = string.IsNullOrWhiteSpace(Config.cacheFolder) ? IOUtils.PathCombine(GetStorageFolder(), "Extracted") : Config.cacheFolder;
            return cachePath;
        }
        ...
        public static async Task<string> EnsureMaterializedAsset(Asset asset, AssetFile assetFile = null)
        {
            ...
            if (!string.IsNullOrEmpty(asset.Location) && !File.Exists(asset.Location)) return null;
        ...
        public static async void RefreshIndex(int assetId = 0, bool force = false)
        {
            ...
            if (Config.indexAssetStore && assetId == 0)
            {
                ...
                {
                    await new UnityPackageImporter().IndexRoughLocal(assetStoreDownloadCache, true, force);
                }
                else
                {
                ...
                switch (spec.folderType)
                {
                    case 0:
                        if (assetId == 0)
                        {
                            ...
                            await new UnityPackageImporter().IndexRoughLocal(spec.location, hasAssetStoreLayout, force);
                        }
            ...
            if (Config.indexAssetStore && Config.downloadAssets && assetId == 0)
            {
                ...
                if (Directory.Exists(assetStoreDownloadCache))
                {
                    List<AssetInfo> assets = LoadAssets()
                        .Where(info => info.AssetSource == Asset.Source.AssetStorePackage && !info.Downloaded && !info.IsAbandoned && !info.IsIndexed && !string.IsNullOrEmpty(info.OfficialState))
                        .ToList();
